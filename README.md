...
6. Configurar un servicio externo de señal de vida con avisos a correo/móvil. Debe ser independiente de GitHub. `HEARTBEAT_URL` es la dirección HTTPS que acepta una señal POST. Configurar inicialmente vencimiento de 30 horas: cubre el pase diario del fin de semana, pero no detecta una caída de mañana hasta la jornada siguiente. Para cumplir la meta de aviso en la misma jornada hay que configurar además un horario externo compatible con Europe/Madrid: señal esperada cada hora de 8 a 20 laborables, tolerancia 2 horas, y una diaria a las 9 los fines de semana. No declarar aceptación mientras esto no se compruebe.
...
Los avisos se retienen si falla SMTP y se reintentan. Si el servidor acepta un correo pero el proceso muere antes de guardar estado, podría repetirse; se prioriza no perderlo. El programa no expone secretos en los informes.
